# PowerShell script to register the Weekly FTO Patent Watch task in Windows Task Scheduler
...
if (-not $isAdmin) {
    ...
    Write-Host "Please close this shell, open a new PowerShell window as Administrator, and try again." -ForegroundColor Red
    Exit 1
}

# 2. Define directory paths
$WorkingDir = $PSScriptRoot
$ScriptPath = Join-Path $WorkingDir "ip_watchdog.py"
$TaskName = "BiopharmaWeeklyFTOPatentWatch"

Write-Host "-----------------------------------------------------------------" -ForegroundColor Cyan
Write-Host "Configuring Weekly Windows Task Scheduler automation for FTO Watch" -ForegroundColor Cyan
Write-Host "-----------------------------------------------------------------" -ForegroundColor Cyan

# 3. Locate Python executable path
$PythonPath = Join-Path $WorkingDir "..\biopharma-radar\.venv\Scripts\python.exe"
if (Test-Path $PythonPath) {
    Write-Host "Found local virtual environment Python from radar: $PythonPath" -ForegroundColor Green
} else {
...
Write-Host "Setting up Scheduled Task actions & trigger (8:00 AM every Monday)..." -ForegroundColor Yellow
...
try {
    Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $Trigger -Settings $Settings -Description "Automated weekly FTO patent watch and competitor filing alerts." -Force -ErrorAction Stop
    Write-Host "`nSUCCESS: Scheduled task '$TaskName' has been registered." -ForegroundColor Green
    Write-Host "It will run weekly at 8:00 AM every Monday morning." -ForegroundColor Green
